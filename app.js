...
function renderTiming(task){
  const display=document.createElement("div");display.className="timing-display";const orb=document.createElement("div");orb.className="timing-orb";display.append(orb);const button=document.createElement("button");button.type="button";button.className="signal-button";button.textContent="スタート";let started=0;const seconds=task.targetSeconds||3,target=seconds*1000,tolerance=task.toleranceMs||650;
  button.addEventListener("click",()=>{if(!started){started=performance.now();button.textContent=`${seconds}秒だと思ったら、いま！`;orb.classList.add("running");$("question-help").textContent="数字は出ません。己を信じて。";clearTimeout(deadlineTimeout);cancelAnimationFrame(timerRaf);startDeadline(target+2200,()=>finishTask(false,{detail:`${seconds+2}秒を超えました。時空から戻ってください。`}));return}const elapsed=performance.now()-started,diff=Math.abs(elapsed-target),ok=diff<=tolerance;finishTask(ok,{reactionMs:Math.round(elapsed),quality:clamp(1-diff/(tolerance*2.6),0,1),detail:`あなたの${seconds}秒は ${(elapsed/1000).toFixed(2)} 秒でした。`})});$("challenge").append(display,button);startDeadline(task.duration,()=>finishTask(false,{detail:"スタートする前に時間が終わりました。"}));
}
