...
const TASK_FACTORIES = [
  ...
  {id:"attention-author-boss-v1",version:"1.0",category:"attention",make:()=>({kind:"authorBoss",prompt:"動き続ける赤い中ボスの作者を3回つかまえて",help:"締切から逃走中。赤いアイコンを追いかけてタップ。",hits:3,duration:7000})},
...
function renderAuthorBoss(task){
  const scene=createStage3D("boss-stage","中ボスの作者アイコン"),button=document.createElement("button"),img=document.createElement("img"),counter=document.createElement("span");button.type="button";button.className="boss-target";button.setAttribute("aria-label","動き続ける中ボスの作者");img.src="author.png";img.alt="";counter.className="boss-counter";counter.textContent=`0 / ${task.hits}`;button.append(img);scene.world.append(button,counter);$("challenge").append(scene.root);
  let hits=0,x=0,y=0,vx=0,vy=0;const reducedMotion=matchMedia("(prefers-reduced-motion: reduce)").matches;const limits=()=>{const inset=10;return{minX:inset,minY:inset,maxX:Math.max(inset,scene.world.clientWidth-button.offsetWidth-inset),maxY:Math.max(inset,scene.world.clientHeight-button.offsetHeight-inset)}};const paint=()=>{button.style.setProperty("--boss-x",`${x.toFixed(1)}px`);button.style.setProperty("--boss-y",`${y.toFixed(1)}px`);button.style.setProperty("--boss-turn",`${clamp(vx/10,-14,14).toFixed(1)}deg`)};const place=()=>{const edge=limits();x=edge.minX+randomFloat()*(edge.maxX-edge.minX);y=edge.minY+randomFloat()*(edge.maxY-edge.minY);paint()};const redirect=()=>{const boost=hits*12;vx=(randomFloat()<.5?-1:1)*(125+randomFloat()*35+boost);vy=(randomFloat()<.5?-1:1)*(90+randomFloat()*30+boost)};place();redirect();
  if(!reducedMotion){let last=performance.now();const token={id:null};extraRafs.push(token);const tick=now=>{if(questionAnswered)return;const dt=Math.min(Math.max((now-last)/1000,0),.05),edge=limits();last=now;x+=vx*dt;y+=vy*dt;if(x<=edge.minX){x=edge.minX;vx=Math.abs(vx)}else if(x>=edge.maxX){x=edge.maxX;vx=-Math.abs(vx)}if(y<=edge.minY){y=edge.minY;vy=Math.abs(vy)}else if(y>=edge.maxY){y=edge.maxY;vy=-Math.abs(vy)}paint();token.id=requestAnimationFrame(tick)};token.id=requestAnimationFrame(tick)}
  button.addEventListener("click",()=>{hits++;counter.textContent=`${hits} / ${task.hits}`;button.classList.remove("bonk");void button.offsetWidth;button.classList.add("bonk");if(hits>=task.hits){finishTask(true,{quality:clamp(1-(performance.now()-questionStartedAt)/task.duration,0,1),detail:"作者を締切へ戻しました。"});return}if(reducedMotion)place();else redirect()});startDeadline(task.duration,()=>finishTask(false,{detail:"作者は締切の向こうへ逃げました。"}));
}
