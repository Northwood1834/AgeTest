...
function renderTask(task){
  ...
  if(task.kind==="target"){
    const arena=document.createElement("div");arena.className="target-arena";const target=document.createElement("button");target.type="button";target.className="moving-target";target.setAttribute("aria-label","紫の的");target.style.left=`${task.x}%`;target.style.top=`${task.y}%`;target.hidden=true;let appeared=0,caught=false;const catchTarget=event=>{event.preventDefault();event.stopPropagation();if(caught||questionAnswered||!appeared)return;caught=true;const ms=performance.now()-appeared;finishTask(true,{reactionMs:Math.round(ms),quality:clamp(1-ms/4000,0,1),detail:`確保まで ${Math.round(ms)} ms`})};target.addEventListener("pointerdown",catchTarget);target.addEventListener("click",catchTarget);arena.addEventListener("click",event=>{if(event.target===target||target.contains(event.target))return;finishTask(false,{detail:"そこにはもう、紫はいません。"})});arena.append(target);root.append(arena);later(()=>{appeared=performance.now();target.hidden=false},450);startDeadline(task.duration,()=>finishTask(false,{detail:"紫は逃げ切りました。"}));return
  }
