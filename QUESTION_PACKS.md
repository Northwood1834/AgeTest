...
Each of these minigame families ships as its basic form — the easiest tuning of its mechanic. Harder versions (a 6×6 link board, a four-colour sort, four plates of bolts, four pins) belong in later packs as new stable IDs at Tier 3, never as a retune of an existing ID, so a player's history keeps its meaning. The date task randomly uses one of three fictional adult characters and requires three context-aware choices before the invitation succeeds. `language-english-v1` presents its prompt, help, answers, and distractors entirely in English.
