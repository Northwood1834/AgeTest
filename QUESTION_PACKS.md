...
| Block stacking | 5–6 levels | 8 levels, faster slide | The date task randomly uses one of three fictional adult characters and requires three context-aware choices before the invitation succeeds. `language-english-v1` presents its prompt, help, answers, and distractors entirely in English, and every English family follows that rule. `language-word-order-v1` is a sentence builder: five chunks are tapped into place, a placed chunk can be tapped back, and the sentence is judged the moment the last chunk lands. Its sixteen sentences each hinge on one grammar point — present perfect, indirect questions, so/too constructions, would rather, first conditional — and were chosen so that only one order is correct. Three more English families follow the same rule: `language-english-gap-v1` fills a blank, `language-english-form-v1` puts a bracketed verb into the right form, and `language-english-error-v1` asks which of four sentences is wrong — the last sits at Tier 3 because comparing four sentences carries the most load.

Tier 1 contains intentionally simple, quickly understood families for new players; v1.16.0 widened it from 18 to 29 so the opening sets stay ordinary as the specialised families grow — subtraction, halving, kanji readings, everyday wording, a three-colour recall, counting marks, an alternating pattern, a mirrored arrow, a two-second clock, naming a shape, and picking the natural greeting. Tier 2 and Tier 3 stay out of selection until the player's brain level reaches the matching tier. A session selects 12 eligible factories, seeds at least one eligible family from each play flavor, caps one category at two questions, prioritizes unseen and weaker categories, and penalizes recently used factories. The author-icon mid-boss is inserted at position 6 only at brain level 2 or above. The final order is cryptographically shuffled before that insertion, so the next category is not previewed.
